...
sys_exofork(void)
{
    ...
    struct Env *new_env;
    int error_code = env_alloc(&new_env, curenv->env_id);
    if (error_code < 0) {
	return error_code;
    } else {
	// envs[new_env->env_id].env_status = ENV_NOT_RUNNABLE;
	envs[ENVX(new_env->env_id)].env_status = ENV_NOT_RUNNABLE;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
    ...
    if (page_ptr == NULL ||
	page_insert(env_ptr->env_pgdir, page_ptr, va, perm) < 0) {
	page_free(page_ptr);
	return -E_NO_MEM;
    }
    return 0;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
    ...
    // LAB 4: Your code here.
    struct Env* srcenv_ptr = NULL;
    struct Env* dstenv_ptr = NULL;
    if (envid2env(srcenvid, &srcenv_ptr, 1) < 0 ||
	envid2env(dstenvid, &dstenv_ptr, 1) < 0)
	return -E_BAD_ENV;
    if ((unsigned)srcva >= UTOP || (unsigned)srcva % PGSIZE != 0 || 
	(unsigned)dstva >= UTOP || (unsigned)dstva % PGSIZE != 0)
	return -E_INVAL;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
    // LAB 4: Your code here.
    struct Env *dstenv_ptr = NULL;
    if (envid2env(envid, &dstenv_ptr, 0) < 0)
	return -E_BAD_ENV;
	
    if (!dstenv_ptr->env_ipc_recving)
	return -E_IPC_NOT_RECV;

    if ((unsigned)srcva < UTOP && (unsigned)(dstenv_ptr->env_ipc_dstva) < UTOP) {
	if ((unsigned)srcva % PGSIZE != 0)
	    return -E_INVAL;
	if ((perm & (~PTE_SYSCALL)) != 0 || 
	    (perm & (PTE_U | PTE_P)) != (PTE_U | PTE_P))
		return -E_INVAL;
    ...
    dstenv_ptr->env_ipc_recving = false;
    dstenv_ptr->env_ipc_from = curenv->env_id;
    dstenv_ptr->env_ipc_value = value;
    dstenv_ptr->env_status = ENV_RUNNABLE;
    // "return" value from calling sys_ipc_recv in target environment
    dstenv_ptr->env_tf.tf_regs.reg_eax = 0;
    return 0;
